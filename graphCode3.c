...
vector<graphEdge> primEdgeFinder(const Graph& g, int startVertex){
    cout << "Dealing with start vertex of " << startVertex << endl;
...
void Problem1::stop(int id, Graph &G, Forest &MTidForest) {
    ...
    for (auto& verticeSource : tree.V) {
        cout << "Cost " << tree.ct << " for id " << tree.id << endl;
        int V = G.V.size();
        vector<int> key(V, INT_MAX);
        vector<bool> inMST(V, false);
        inMST[verticeSource] = true;
        vector<graphEdge> pathsConnected = primEdgeFinder(G, verticeSource);

        cout << "Vertice visited\n";
        ...
        for (auto& edge : pathsConnected) {
            cout << "Connected edges are " << edge.vertex[0] << " -> " << edge.vertex[1] << endl;
            minHeap.push(edge);
        }
        ...
        while (!minHeap.empty()) {
            ...
            cout << "New vertex of " << newVertex << " is pushed back" << endl;
            //Adjust edge value
            currentEdge.b -= traffic;

            vector<graphEdge> newPathsConnected = primEdgeFinder(G, newVertex);

            for (auto& edge : newPathsConnected) {
            cout << "new Connected edges are " << edge.vertex[0] << " -> " << edge.vertex[1] << endl;
            minHeap.push(edge);
        }
